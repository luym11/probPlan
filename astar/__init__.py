    ...
    def astar(self, start, goal, reversePath=False):
        ...
        while openSet:
            ...
            if self.is_goal_reached(current.data, goal):
                return self.reconstruct_path(current, reversePath), self.searchNodes, current
            current.out_openset = True
            current.closed = True
            for neighbor in [self.searchNodes[n] for n in self.neighbors(current.data)]:
                if neighbor.closed:
                    continue
